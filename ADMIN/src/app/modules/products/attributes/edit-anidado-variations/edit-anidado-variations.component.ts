...
export class EditAnidadoVariationsComponent {
    ...
    @Input () is_variation:any;//cuando es False va a ser una edicion para las especificaciones y si es True una edicion para la variacion.
    ...
    if (this.type_attribute_specification == 4 && this.selectedItems.length == 0) {
      this.toastr.error('Validacion', 'Seleccione al menos una propiedad');
      return;
    }
    ...
    if (!this.specification_attribute_id || (!this.propertie_id && !this.value_add)) {
      this.toastr.error('Validacion', 'Complete los campos requeridos');
      return;
    }
      ...
      this.attributeService.updateSpecification(this.specification.id, data).subscribe((resp:any) => {
        console.log(resp);
          if (resp.message == 403) {
            ...
            this.toastr.success('Guardado', 'Especificación actualizada correctamente');
        ...
        this.attributeService.updateVariationsAnidadas(this.specification.id, data).subscribe((resp:any) => {
          console.log(resp);
            if (resp.message == 403) {
              ...
              this.toastr.success('Guardado', 'Variacion anidada actualizada correctamente');
              this.EspecificationE.emit(resp);
              this.modal.close();
            }
        })
