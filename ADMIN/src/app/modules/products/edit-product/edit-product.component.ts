...
export class EditProductComponent {
  ...
  addImagen() {
    if (!this.imagen_add) {
      this.toastr.error('Validacion', 'Debe subir una imagen');
      return;
    }
  ...
  save() {
    ...
    if(!this.title || !this.sku || !this.price_usd || !this.price_cop || !this.marca_id
      ...
      this.toastr.error('Validacion','Todos los campos con * son obligatorios');
      return;
    }
    ...
    this.productService.updateProducts(this.PRODUCT_ID, formData).subscribe((resp:any) => {
      ...
      if (resp.message == 403) {
        ...
        this.toastr.success('Exito', 'Producto actualizado con éxito');
      }
    });
  }
}
