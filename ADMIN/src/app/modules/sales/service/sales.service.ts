...
export class SalesService {
  ...
  reportSaleForCountry(data: any) {
    ...
    let URL = URL_SERVICIOS + '/admin/kpi/report_sale_country_for_year';
    return this.http
      .post(URL, data, { headers: headers })
      .pipe(finalize(() => this.isLoadingSubject.next(false)));
  }
  ...
  reportSaleForCategories(data:any){
    ...
    let URL = URL_SERVICIOS+"/admin/kpi/report_sale_categories";
    return this.http.post(URL,data,{headers: headers}).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }

  reportSaleForCategorieDetails(data:any) {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({'Authorization': 'Bearer '+this.authservice.token});
    let URL = URL_SERVICIOS+"/admin/kpi/report_sale_categories_details";
    return this.http.post(URL,data,{headers: headers}).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }
