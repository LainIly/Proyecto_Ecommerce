...
<div class="web-container">
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td style="padding: 0 !important;border-bottom:none;">
                    <br>
                    <small>Tel. 318 3422398</small>
                    <br>
                    <small>Km. 6,5 via Daule Lotizacion Santa <br> Jaime Andres Guacarapare Sotelo Mz. 25 Solar 11 090505 <br> Huila, Colombia </small>
                </td>
            </tr>
        </tbody>
    </table>
  ...
  <table class="invoice-info-container">
    ...
    <tr>
      ...
      <td class="">
        ...
        <br>
        <br>
        CLIENTE: {{ $sale->user->name.' '.$sale->user->lastname }}
        <br>
    ...
    <tr>
        <td>
            <b>Datos de Envío:</b>
            <br>
            DIRECCIÓN: {{ $sale->sale_addres->street }}<strong></strong>
            <br>
            EMPRESA: {{ $sale->sale_addres->company }}<strong></strong>
            <br>
            REGION/CIUDAD: {{ $sale->sale_addres->country_region }}<strong></strong>
            <br>
            CIUDAD: {{ $sale->sale_addres->city }}<strong></strong>
        </td>
        <td></td>
    </tr>
  ...
  <table class="line-items-container">
    ...
    <tbody>
      ...
      @php
        if (!function_exists('formatPriceToCOP')) {
      ...
      @foreach ($sale->sale_details as $item)
        <tr>
            ...
            @if ($item->discount > 0)
                                    <br />
                <strong>Descuento: - {{ formatPriceToCOP($item->discount) }} {{ $item->type_discount == 1 ? '%' : $item->currency }}</strong>
            ...
            <td class="right">{{ formatPriceToCOP($item->subtotal) }} {{ $item->currency }}</td>
            <td class="bold">{{ formatPriceToCOP($item->total) }} {{ $item->currency }}</td>
        </tr>
      @endforeach
    </tbody>
  </table>
    ...
    <tbody>
      <tr>
        <td class="payment-info">
            <div>
                METODO DE PAGO: {{ $sale->method_payment }}<strong></strong>
            </div>
        </td>
        ...
        <td class="payment-info">
            <div class="large total">
                TOTAL: {{ formatPriceToCOP($sale->total) }} {{ $sale->currency_payment }}
            </div>
        </td>
    </tbody>
  </table>
