...
class KpiSaleReportController extends Controller
{
    ...
    public function report_sale_country_for_year (Request $request) { //Reporte de ventas por año y país.
        ...
        $query->select("sale_addres.country_region as country_region",
                DB::raw('ROUND(SUM(IF(sales.currency_payment = "USD", sales.total*3800, sales.total)), 2) as total_sales'))
    ...
    public function report_sales_week_discounts () {  //Reporte de descuentos aplicados en la semana actual vs semana pasada.
        $start_week = Carbon::now()->startOfWeek();
        $end_week = Carbon::now()->endOfWeek();
    ...
    public function report_sales_month_selected (Request $request) {
        ...
        if ($sales_for_month_last) {
            $porcentajeV = (($sales_for_day_of_month->sum('sales_total') - $sales_for_month_last) / $sales_for_month_last)*100;
        }
    ...
    public function report_sales_for_month_year_selected (Request $request) {
        ...
        $query = DB::table('sales')->where('sales.deleted_at', NULL) //An/o actual
                                    ->whereYear('sales.created_at', $year)
                                    ->select(
        ...
        $query_discount = DB::table('sales')->where('sales.deleted_at', NULL) //An/o actual
                                    ...
                                    ->select(
                                        DB::raw('ROUND(SUM(IF(sale_details.currency = "USD", sale_details.discount*3800, sale_details.discount)), 2) as discount_total'),
                                        DB::raw('COUNT(*) as count_total')
                                    )
                                    ->get();
        
        $query_cupone = DB::table('sales')->where('sales.deleted_at', NULL) //An/o actual
                                    ->join('sale_details', 'sale_details.sale_id', '=', 'sales.id')
                                    ->where('sale_details.deleted_at', NULL)
                                    ->whereYear('sales.created_at', $year)
    ...
    public function report_sale_categories (Request $request) {
        $year = $request->year;
        $month = $request->month;
    ...
    public function report_sale_categories_details (Request $request) {
        $year = $request->year;
        $month = $request->month;
        ...
        $sales_month_categories = DB::table('sales')->where('sales.deleted_at', NULL)
                    ...
                    ->select('categories.name as categorie_name',
                        ...
                        DB::raw('ROUND(SUM(IF(sale_details.currency = "USD", sale_details.total*3800, sale_details.total)), 2) as categorie_total'))
        ...
        foreach($sales_month_categories as $key => $sales_month_categ) {
                    ...
                    ->select('products.title as product_title', 'products.sku as product_sku', 'products.price_cop as product_price_cop', 'products.imagen as products_imagen',
                        DB::raw('ROUND(SUM(IF(sale_details.currency = "USD", sale_details.total*3800, sale_details.total)), 2) as product_total'),
                        DB::raw('ROUND(SUM(sale_details.quantity), 2) as product_quantity_total'))
                    ->groupBy('product_title', 'product_sku', 'product_price_cop', 'products_imagen') //
            ...
            $product_most_sales->push([
                ...
                'products' => $query_product_most_sales->map(function($item) {
                    $item->imagen = env('APP_URL').'storage/'.$item->products_imagen;
                    return $item;
                }),
            ]);
        }
    ...
    public function report_sale_brands (Request $request) {
        $year = $request->year;
        $month = $request->month;
